...
use Plugin\s360_barzahlen_shop4\lib\Helper;
use Plugin\s360_barzahlen_shop4\lib\APIClient;
use Plugin\s360_barzahlen_shop4\lib\Database;
use Shop;
...
class Barzahlen extends PaymentMethod 
{
    ...
    {   
        ...
        (Helper::isset_noempty($_SESSION["Waehrung"]->cISO))
            ? $this->cWaehrungName = $_SESSION["Waehrung"]->cISO
    ...
    {      
        ...
        $targetLocale = strtolower(StringHandler::convertISO2ISO639(Shop::getLanguage(true))) . '-' . strtoupper(isset($order->oRechnungsadresse->cLand) ? $order->oRechnungsadresse->cLand : '');
        ...
        if (Helper::isset_noempty($this->config->sendCustomerAddress)) {
            if ($order->oRechnungsadresse->cStrasse && $order->oRechnungsadresse->cHausnummer) {
                $address["street_and_no"] = html_entity_decode(utf8_encode($order->oRechnungsadresse->cStrasse . " " . $order->oRechnungsadresse->cHausnummer), ENT_COMPAT, 'UTF-8');
            }
            $address["zipcode"] = $order->oRechnungsadresse->cPLZ;
            $address["city"] = html_entity_decode(utf8_encode($order->oRechnungsadresse->cOrt), ENT_COMPAT, 'UTF-8');
            $address["country"] = $order->oRechnungsadresse->cLand;
            $request->setAddress($address);
        }        
        $request->setHookUrl(Database::getInstance()->getWebhookUrl());
        $request->setTransaction(number_format($order->fGesamtsummeKundenwaehrung, 2), $this->cWaehrungName);
        ...
        try {
            ...
            $tBestellId = Shop::DB()->select('tbestellid', 'kBestellung', (int)$orderFinalized->kBestellung);
    ...
    public function isValidIntern($args_arr = []) {
        ...
        if (!$this->config->tZahlungsart->nWaehrendBestellung) {
    ...
    private function getAdditionalPaymentCost() {
        ...
        if (Helper::isset_noempty($_SESSION['AktiveVersandart'])) {
            $tVersandartZahlungsart = Shop::DB()->select('tversandartzahlungsart', 'kVersandart', (int)$_SESSION['AktiveVersandart'], 'kZahlungsart', $this->config->tZahlungsart->kZahlungsart);

...
class Admin {
    ...
    public function __construct() {
        ...
        $this->plugin = Plugin::getPluginById(Config::PluginID);
    ...
    private function handleDisplay() {
        ...
        Shop::Smarty()->assign('AdminmenuPfad', $this->plugin->cAdminmenuPfad);
        Shop::Smarty()->assign('AdminmenuPfadURLSSL', $this->plugin->cAdminmenuPfadURLSSL);
        Shop::Smarty()->assign('AdminPluginURL', Shop::getURL(true) . '/admin/plugin.php?kPlugin=' . $this->plugin->kPlugin);
        Shop::Smarty()->display($this->plugin->cAdminmenuPfad . "template/payments.tpl");
    }
    ...
    private function searchSlips($search) {
        try {
            ...
            $html = Shop::Smarty()->fetch($this->plugin->cAdminmenuPfad . 'template/payments_inc.tpl');
            return [
                'result' => 'success',
                'html' => $html
            ];
    ...
    private function getRefundForm($slip_id) {
        try {
            ...
            $html = Shop::Smarty()->fetch($this->plugin->cAdminmenuPfad . 'template/refund.tpl');
    ...
    private function getSlipInfo($slip_id) {
        try {
            ...
            Shop::Smarty()->assign('slip', $slip);
            $html = Shop::Smarty()->fetch($this->plugin->cAdminmenuPfad . 'template/slip.tpl');

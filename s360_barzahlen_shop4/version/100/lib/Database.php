...
class Database {
    ...
    const TABLE_NAME_SLIP = "xplugin_s360_barzahlen_shop4_slip";
    ...
    public function getWebhookUrl() {
        $kSprache = (int)Shop::getLanguage(false);
        if ($kSprache <= 0) {
            $kSprache = (int)gibStandardsprache(true)->kSprache;
        }
        if (!empty($kSprache)) {
            $kPlugin = Config::getInstance()->tPlugin->kPlugin;                
